...
## Tocat > service-account-csr.json <<EOF
{
  "CN": "service-accounts",
  "key": {
    "algo": "rsa",
...
2Now it is time to start sending all the client and server certificates to their respective instances.
...
7. Create the etcd.service systemd unit file: The flags are well documented here
```
...
5. Configure the Kubernetes Controller Manager: Move the kube-controller-manager kubeconfig into place:
...
spec:
  ...
  ingress:
   ...
   ports:
     ...
     port: 3306
```
3. Disable Swap If swap is not disabled, kubelet will not start. It is highly recommended to allow Kubernetes to handle resource allocation.
...
9. Configure kubelet: In the home directory, you should have the certificates and kubeconfig file for each node. A list in the home folder should look like below:
...
10. Configure the bridge and loopback networks Bridge:
...
```
14. Create the kubelet-config.yaml file Ensure the needed variables exist:
