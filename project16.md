...
- You must have completed [Terraform course](https://www.udemy.com/) from the Learning dashboard
...
To destroy whatever has been created run `terraform destroy` command, and type `yes` after evaluating the plan or use `terraform --auto-approve`.
...
Since `data.aws_availability_zones.available.names` returns a list like `["eu-central-1a", "eu-central-1b", "eu-central-1c"]` we can pass it into a `lenghth` function and get number of the AZs
